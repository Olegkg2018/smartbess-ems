...
class MarketBid(Base):
    ...
    # Реальні дані звірки з балансуючим ринком (БР) — 2026-09-09, формула й
    # склад полів відтворені з реального облікового Excel-файлу справжнього
    # підприємства з батареєю ("УЗЕ Флора", наданий користувачем): дефіцит/
    # профіцит між тим, що РЕАЛЬНО пройшло через лічильники (заряд/розряд/
    # власні потреби), і тим, що було заявлено на РДН+ВДР, закривається на
    # БР за окремою ціною. Диспетчер вводить ці 5 полів ВРУЧНУ, коли отримує
    # реальний рахунок/акт звірки від постачальника чи оператора системи
    # передачі — офіційного живого джерела цих даних немає (MEMORY.md,
    # "Розвідка реальних цін небалансу — заблоковано"). Усі nullable —
    # None = ще не звірено (чесно немає даних, не вигаданий 0); небаланс
    # рахується (bidding_service.py::compute_imbalance_financials) лише
    # коли всі три "Факт"-поля заповнені.
    actual_charge_mwh = Column(Float, nullable=True)           # Факт Заряд (А+), реальний лічильник
    actual_discharge_mwh = Column(Float, nullable=True)        # Факт Розряд (А-), реальний лічильник
    actual_own_consumption_mwh = Column(Float, nullable=True)  # Факт Власні потреби (А+) — обслуговування/КВП батареї
    balancing_sell_price_uah = Column(Float, nullable=True)    # Ціна продажу БР (за профіцит-небаланс), ₴/МВт·год
    balancing_buy_price_uah = Column(Float, nullable=True)     # Ціна докупки БР (за дефіцит-небаланс), ₴/МВт·год
