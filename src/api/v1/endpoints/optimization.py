...
def run_optimization_background_job(
    ...
    try:
        ...
        # 2026-08-26: межа "минуле/майбутнє" — це КІНЕЦЬ години (t+1), а не її
        # ПОЧАТОК. Раніше межа була "початок години <= зараз" — це заморожувало
        # ГОДИНУ, ЩО ЩЕ ТРИВАЄ ПРЯМО ЗАРАЗ, хоча SCADA-контур (poll_bess_and_control,
        # кожні 10с) досі активно керує саме нею й одразу підхопить оновлену
        # команду. Знайдено користувачем: перерахунок о 12:13 заморозив
        # поточну (ще не завершену) годину зі старим "чистим" SoC (3.6
        # МВт·год), а жива телеметрія тим часом лишалась на реальних 24% —
        # диспетчер бачив розбіжність САМЕ на поточній годині, не в майбутньому.
        ...
        if force_full_day:
            ...
            start_t = 24
            for t in range(24):
                if kyiv_to_utc(target_date_str, t + 1) > now_utc:
                    start_t = t
                    break
            if start_t == 24:
                raise RuntimeError(
